...
class LearnedRouter(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        raw_expert_weights, expert_indices = self._top_k(scores)
        if self.args.moe_normalize_expert_weights:
            expert_weights = raw_expert_weights / torch.norm(
                raw_expert_weights, p=self.args.moe_normalize_expert_weights,dim=-1, keepdim=True)
        else:
            expert_weights = raw_expert_weights
